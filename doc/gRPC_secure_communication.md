...
The navigation server framework provides the necessary tools to fully set up gRPC over TLS to secure its operations.
...
**Note1: Remember to share the CA certificate with all clients**

**Note2: CA key and certificate need to be generated only once for the whole system, so don't generate them on each server**
...
3) Generate the server key and certificate according to the SSL configuration file.
...
The SSL key location can be also controlled by the environment variable NAV_CONF_DIR that defaults to $HOME.
